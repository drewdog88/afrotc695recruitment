#!/usr/bin/env python3
"""
Script to restore data from Neon backup with proper PostgreSQL syntax
"""
...
def restore_users(conn, users_data):
    ...
    for user in users_data:
        try:
            # Handle PostgreSQL reserved word 'user' with quotes
            cursor.execute("""
                INSERT INTO "user" (
                    ...
                    %(secret_question)s, %(secret_answer_hash)s, %(totp_secret)s, %(totp_enabled)s,
                    %(backup_codes_hash)s, %(totp_setup_completed)s, %(can_enable_2fa)s,
                    %(created_at)s, %(last_modified)s
                    ...
                    totp_setup_completed = EXCLUDED.totp_setup_completed,
                    can_enable_2fa = EXCLUDED.can_enable_2fa,
...
def restore_cadets(conn, cadets_data):
    ...
    for cadet in cadets_data:
        try:
            cursor.execute("""
                INSERT INTO cadet (
                    id, first_name, last_name, email, phone, rank, class_year,
                    major, gpa, status, notes, created_at, updated_at
                ) VALUES (
                    %(id)s, %(first_name)s, %(last_name)s, %(email)s, %(phone)s, %(rank)s, %(class_year)s,
                    %(major)s, %(gpa)s, %(status)s, %(notes)s, %(created_at)s, %(updated_at)s
                ) ON CONFLICT (id) DO UPDATE SET
                    first_name = EXCLUDED.first_name,
                    last_name = EXCLUDED.last_name,
                    email = EXCLUDED.email,
        ...
        try:
            cursor.execute("""
                INSERT INTO external_link (
                    id, title, url, description, category, created_at, updated_at
                ) VALUES (
                    %(id)s, %(title)s, %(url)s, %(description)s, %(category)s, %(created_at)s, %(updated_at)s
                ) ON CONFLICT (id) DO UPDATE SET
                    title = EXCLUDED.title,
                    url = EXCLUDED.url,
                    description = EXCLUDED.description,
        ...
        try:
            cursor.execute("""
                INSERT INTO activity_log (
                    ...
                    %(id)s, %(user_id)s, %(username)s, %(action)s, %(table_name)s, %(record_id)s,
...
def main():
    ...
    print("=== Neon Backup Data Restoration ===")

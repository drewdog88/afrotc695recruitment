    ...
    try:
        ...
        for obj in response.get('Contents', []):
            ...
            if filename.endswith('.json') and 'backup' in filename:
                ...
                if last_modified:
                    # Convert to timezone-naive datetime for sorting
        ...
        # Sort by last modified (newest first), handle None values
        backup_files.sort(key=lambda x: x['last_modified'] if x['last_modified'] else datetime.min, reverse=True)
        return backup_files
...
def main():
    ...
    for i, backup in enumerate(backup_files[:10], 1):  # Show last 10
        ...
        last_modified_str = backup['last_modified'].strftime('%Y-%m-%d %H:%M:%S') if backup['last_modified'] else 'Unknown'
        print(f"   📅 {last_modified_str} | 📦 {backup['size']} bytes")
